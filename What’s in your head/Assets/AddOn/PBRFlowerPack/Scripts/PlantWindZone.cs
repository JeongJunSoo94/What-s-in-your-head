...
public class PlantWindZone : MonoBehaviour
{
    ...
    { //Logs some helpful warning to users.
        ...
        {
            Debug.LogWarning("PlantWindZone: Plant Layer is currently set to Nothing, Default, or Everything. It is suggested to use a seperate plant layer for plant collisions. If nothing is used, no wind will be applied.");
        }
        if (!this.gameObject.isStatic)
        {
            Debug.LogWarning("PlantWindZone: " + this.gameObject.name + " is not set to static, it will not apply wind properly to rigidbody colliders if it moves away from the plants. It is suggested to mark as a static gameobject.");
        }
